...
from .validation import AuthSmsSendIn, LoginForm, MPGuestLoginForm, WXLoginForm
...
@app.post(
    "/sms/send",
    summary="登录短信验证码",
    description="小程序等场景：不要求手机号已注册；频控与有效期同阿里云短信配置",
)
async def auth_login_sms_send(
    body: AuthSmsSendIn,
    rd: Redis = Depends(redis_getter),
):
    sms = CodeSMS(body.telephone, rd)
...
@app.post(
    "/login",
    summary="密码/短信登录",
    description="密码登录支持手机号或账号；短信登录：platform=1 时先验码，手机号不存在则自动建号；platform=0 时须已注册。输错次数达上限将 is_active=False",
)
async def login_for_access_token(
    ...
    try:
        if data.method == "0":
            ...
            if data.platform == "1":
                result = await manage.mp_sms_login_with_register(data, db, request)
            else:
...
@app.post("/mp/guest", summary="小程序游客登录（wx.login code，无手机号）")
async def mp_guest_login(
    request: Request,
    data: MPGuestLoginForm,
    db: AsyncSession = Depends(db_getter),
    rd: Redis = Depends(redis_getter),
):
    try:
        if data.platform != "1" or data.method != "4":
            raise ValueError("无效参数")
        wx = WXOAuth(rd, 0)
        sess = await wx.get_code2session(data.code)
        openid = (sess or {}).get("openid")
        if not openid:
            raise ValueError("无效Code")
        user = await UserDal(db).get_or_create_mp_guest(openid)
        if getattr(user, "is_blocked", False):
            raise ValueError("此账号已被拉黑")
        if not user.is_active:
