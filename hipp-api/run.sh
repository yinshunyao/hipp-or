...
try:
    ...
    print('[run.sh] 请在 .env 中设置 HIPP_DATABASE_URL 或分项变量（见 .env.example）。', file=sys.stderr)
    sys.exit(1)
"
}
...
ensure_db_schema_before_run() {
  if [[ -n "${HIPP_SKIP_AUTO_INIT:-}" ]]; then
    echo "[run.sh] 已设置 HIPP_SKIP_AUTO_INIT，跳过缺表自动初始化"
    return 0
  fi
  local rc=0
