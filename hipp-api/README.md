...
- 根据模型配置你的alembic：
  ...
  [dev]
  ...
  sqlalchemy.url = mysql+pymysql://your_username:password@ip:port/hipp
  ...
  ```
  ```python
  # alembic/env.py
...
- 执行数据库迁移命令（终端执行执行脚本）：
  ```shell
  # 默认使用 alembic.ini 中 [pro]（迁移脚本目录：alembic/versions_pro）
  python main.py migrate

  # 使用 [dev]（迁移脚本目录：alembic/versions_dev）——若库表 alembic_version 来自 dev 链，必须用此命令，否则会报
  ...
  **说明**：`dev` 与 `pro` 是两套独立的迁移目录（见 `alembic.ini` 的 `version_locations`）。数据库里 `alembic_version` 记录的是哪条链上的 revision，后续迁移就必须用同一套 `--name`（例如库里已是 `versions_dev` 中的 `6b50cc279ff0`，应使用 `python main.py migrate --env dev`，不要用默认的 `pro`）。
...
![image-20230315194534959](https://ktianc.oss-cn-beijing.aliyuncs.com/hipp/public/images/image-20230315194534959.png)
