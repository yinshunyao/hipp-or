...
"""安全警告: 不要在生产中打开调试运行! 由 .env 中 HIPP_DEBUG 控制（未设置默认为 false）。"""
DEBUG = env_bool("HIPP_DEBUG", False)

"""HTTP 服务绑定（uvicorn），在 `.env` 中通过 HIPP_BIND_HOST / HIPP_BIND_PORT 配置"""
BIND_HOST = env_str("HIPP_BIND_HOST", "0.0.0.0")
BIND_PORT = env_int("HIPP_BIND_PORT", 9000)

"""是否开启演示功能：为真时拦截非 GET（白名单除外）。由环境变量 HIPP_DEMO 控制，未设置时默认为 true。"""
DEMO = env_bool("HIPP_DEMO", True)
"""演示功能白名单"""
...
其他项目配置
"""
# 调用 Dify 开放 API（智能客服测试等）时，httpx 是否校验 TLS 证书。内网自签证书可设为 false（仅建议开发/内网）
DIFY_HTTPX_VERIFY = env_bool("HIPP_DIFY_HTTPX_VERIFY", True)
