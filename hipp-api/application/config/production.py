...
实际连接串由环境变量 HIPP_DATABASE_URL 注入（见 application/database_url.py），勿在此文件写死密码。
"""
from application.database_url import resolve_sqlalchemy_async_url

SQLALCHEMY_DATABASE_URL = resolve_sqlalchemy_async_url()

"""
Redis：是否启用由本项与 .env 中 REDIS_DB_ENABLE 决定；
连接串由环境变量 HIPP_REDIS_URL 注入（见 application/redis_config.py），勿在此写死密码。
"""
...
MONGO_DB_NAME = "hipp"
MONGO_DB_URL = f"mongodb://hipp:123456@177.8.0.6:27017/?authSource={MONGO_DB_NAME}"
