...
关键配置从项目根目录 `.env` 注入（启动时加载），见 `.env.example` 与 README。HTTP 绑定：`HIPP_BIND_HOST`、`HIPP_BIND_PORT`。
"""
...
def run():
    """
    启动项目；监听地址与端口由 `.env` 中 HIPP_BIND_HOST、HIPP_BIND_PORT 配置（见 application/settings.py）。
