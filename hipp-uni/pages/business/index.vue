<template>
  <view :class="themeClass" class="page-with-nav">
    ...
    <view class="input-bar" :class="{ 'input-bar--kb-up': keyboardHeight > 0 }">
      <view class="input-shell">
        <textarea v-model="inputText" class="input-text" :placeholder="inputPlaceholder" :placeholder-style="'color:' + tc.text3" :disabled="isReadonlySession || !sessionId" :auto-height="true" :maxlength="4000" :adjust-position="adjustPositionForInput" :cursor-spacing="24" :show-confirm-bar="false" @keyboardheightchange="onKbHeightChange" @keydown="onInputKeydown" />
...
export default {
  mixins: [themeMixin, mpKeyboardOffsetMixin],
  data() {
    return {
  ...
  methods: {
    ...
    async bootstrap() {
      ...
      try { const d = (await getChatSession(this.sessionId)).data || {}; this.sessionTitle = d.display_title || d.title || '商业'; this.sessionAgentStatus = d.agent_status || 'active'; this.sessionTopicClosed = d.is_topic_closed === true }
      catch (e) { this.sessionTitle = '商业'; this.sessionAgentStatus = 'active'; this.sessionTopicClosed = false }
      await this.loadMessages()
    },
    async loadMessages() {
      if (!this.sessionId) return
      try { const res = await getChatMessages(this.sessionId, 1, 100); const list = Array.isArray(res.data) ? res.data : []; this.messages = list.map((m) => ({ id: m.id, role: m.role, content: m.content })); this.$nextTick(() => this.scrollToBottom()) }
